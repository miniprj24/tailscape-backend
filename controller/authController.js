...
const signUp = async (req, res) => {
  ...
  if (!name || !email || !password || !UIOrigin) {
    return res.status(400).json({ message: 'All fields are required, including UIOrigin.' });
  }
...
const login = async (req, res) => {
  ...
  if (!email || !password || !UIOrigin) {
    return res.status(400).json({ message: 'Email, password, and UIOrigin are required' });
  }
