...
export default function BookprivateHospital() {
  const [selectedHospital, setSelectedHospital] = useState('');
  const [privateHospitals, setprivateHospitals] = useState([]);
  ...
  useEffect(() => {
    const fetchHospitals = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/vetHospitals/private');
        const data = await response.json();
        setprivateHospitals(data);
      } catch (error) {
  ...
  const handleFormSubmit = (e) => {
    e.preventDefault();
    const selectedHospitalData = privateHospitals.find(
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-blue-100/80 px-4 py-12 relative overflow-hidden">
      <main className="max-w-4xl mx-auto relative z-10">
        <h1 className="text-4xl font-bold text-center text-gray-800 mb-4">Book private Hospital</h1>
        <p className="text-center text-gray-600 mb-8">
          Select a private hospital from the dropdown to proceed with your booking.
        </p>
        ...
        >
          <div className="mb-6">
            ...
            >
              ...
              {privateHospitals.map((hospital, index) => (
          ...
          {selectedHospital && (
            <div className="bg-blue-50 p-4 rounded-md border border-blue-200 mb-6">
              <h2 className="text-lg font-bold text-gray-800">
                ...
                <span className="text-sm font-normal text-gray-500">
                  ({getHospitalRatingStars(
                    privateHospitals.find((h) => h.name === selectedHospital)?.rating
                  )})
                </span>
              </h2>
              <p className="text-gray-600">
                {privateHospitals.find((h) => h.name === selectedHospital)?.description}
              </p>
              <p className="text-gray-600">
                Location:{' '}
                <span className="text-blue-600">
                  {privateHospitals.find((h) => h.name === selectedHospital)?.location}
                </span>
              </p>
              <p className="text-gray-600">
                Timings:{' '}
                <span className="text-gray-800">
                  {privateHospitals.find((h) => h.name === selectedHospital)?.timings}
                </span>
              </p>
              <p className="text-gray-600">
                Phone Number:{' '}
                <span className="text-gray-800">
                  {privateHospitals.find((h) => h.name === selectedHospital)?.phone}
                </span>
              </p>
              <a
                href={privateHospitals.find((h) => h.name === selectedHospital)?.mapsLink}
                target="_blank"
              ...
              {privateHospitals.find((h) => h.name === selectedHospital)?.website && (
                <a
                  href={privateHospitals.find((h) => h.name === selectedHospital)?.website}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-blue-500 hover:text-blue-700 underline mt-2 block"
                >
